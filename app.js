...
const ItemCtrl = (function(){
  // Item Constructor
  const Item = function(id, name, quantity, price){
    this.id = id;
    this.name = name;
  ...
  const data = {
    // items: [
    //   // {id: 0, name: 'Steak Dinner', quantity: 1200, price: 2000},
    //   // {id: 1, name: 'Cookie', quantity: 400, price: 2000},
    //   // {id: 2, name: 'Eggs', quantity: 300, price: 2000}
    // ],
  ...
  return {
    ...
    updateItem: function(name, quantity, price){
      // quantity to number
      quantity = parseInt(quantity);
  ...
  return {
    populateItemList: function(items){
      ...
      items.forEach(function(item){
        ...
        x<strong> ${item.quantity} </strong><em>Qty</em> 
        :<strong> ${item.price} </strong><em> /plate</em>
...
const App = (function(ItemCtrl, StorageCtrl, UICtrl){
  ...
  const itemAddSubmit = function(e){
    ...
    if(input.name !== '' && input.quantity !== '' && input.price !== ''){
      // Add item
      const newItem = ItemCtrl.addItem(input.name, input.quantity, input.price);
  ...
  const itemUpdateSubmit = function(e){
    ...
    // Update item
    const updatedItem = ItemCtrl.updateItem(input.name, input.quantity, input.price);
